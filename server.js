...
app.get("/app/users", (req, res) => {   
        let stmt = db.prepare("SELECT * FROM userinfo").all();
...
app.get("/app/user/:id", (req, res) => {   
	let stmt = db.prepare("SELECT * FROM userinfo WHERE id = 2").all();
	res.status(200).json(stmt);
});
...
app.patch("/app/update/user/:id", (req, res) => {  
	let stmt = db.prepare("UPDATE userinfo SET user = COALESCE(?,user), pass = COALESCE(?,pass) WHERE id = ?")
	let info = stmt.run(req.body.user, md5(req.body.pass), req.params.id);
	res.status(200).send({message: info.changes + "record updated: ID" + req.params.id + "(200)"});
});
// DELETE a single user (HTTP method DELETE) at endpoint /app/delete/user/:id
app.delete("app/delete/user/:id", (req, res) =>{
	let stmt = db.prepare("DELETE FROM userinfo WHERE id=?");
